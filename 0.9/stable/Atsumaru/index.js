...
var source = (() => {
  ...
  // src/Atsumaru/main.ts
  var main_exports = {};
  __export(main_exports, {
    Atsumaru: () => Atsumaru,
    AtsumaruExtension: () => AtsumaruExtension
  });
  init_buffer();
  ...
  // src/Atsumaru/main.ts
  var baseUrl = "https://atsu.moe/";
  var AtsumaruExtension = class {
    ...
    async getSearchResults(query, metadata) {
      ...
      if (!query.title) {
        ...
        const request2 = { url: apiUrl, method: "GET" };
        const data3 = await this.fetchJson(request2);
        const items2 = [];
        ...
        return {
          items: items2,
          metadata: items2.length > 0 ? { page: page + 1, searchCollectedIds: collectedIds } : void 0
        };
      }
      const searchUrl = new URLBuilder(baseUrl).addPath("api").addPath("search").addPath("page").addQuery("query", query.title.replace(/\s+/g, "+"));
      ...
      return {
        items,
        metadata: items.length > 0 ? { page: page + 1, searchCollectedIds: collectedIds } : void 0
      };
    }
    async getMangaDetails(mangaId) {
      ...
      const tags = [];
      const genres = mangaPage.tags?.map((tag) => tag.name) || [];
      if (genres.length > 0) {
        tags.push({
          id: "genres",
          title: "Genres",
      ...
      if (authors.length > 0) {
        tags.push({
          ...
          tags: authors.map((author) => ({
            id: author.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, ""),
            title: author
          }))
        });
      }
    ...
    async getChapterDetails(chapter) {
      const apiUrl = new URLBuilder(baseUrl).addPath("api").addPath("read").addPath("chapter").addQuery("mangaId", chapter.sourceManga.mangaId).addQuery("chapterId", chapter.chapterId).build();
      const request = { url: apiUrl, method: "GET" };
      const data2 = await this.fetchJson(request);
  ...
  var Atsumaru = new AtsumaruExtension();
