...
var source = (() => {
  ...
  var NHentaiExtension = class {
    ...
    async getSearchResults(query, metadata) {
      ...
      const searchUrl = new URLBuilder(baseUrl).addPath("search").addQuery("q", encodeURIComponent(query.title ?? "")).addQuery("page", page.toString());
      const url = searchUrl.build();
      const request = { url, method: "GET" };
      const $2 = await this.fetchCheerio(request);
      const searchResults = [];
      $2(".container.index-container .gallery").each((_, element) => {
        const gallery = $2(element);
    ...
    async getMangaDetails(mangaId) {
      ...
      const $2 = await this.fetchCheerio(request);
      const title = $2(".title .pretty").first().text().trim();
      const altTitles = [];
      $2(".title").each((_, el) => {
        const alt = $2(el).text().trim();
        if (alt && alt !== title) altTitles.push(alt);
      });
      const image = $2("#cover img").attr("data-src") || $2("#cover img").attr("src") || "";
      const description = "";
      const status = "UNKNOWN";
      const tags = [];
      $2("#tags .tag-container").each((_, el) => {
        const sectionTitle = $2(el).contents().first().text().replace(":", "").trim();
        const tagList = [];
        $2(el).find(".tags a.tag").each((_2, tagEl) => {
          const id = $2(tagEl).attr("href")?.split("/").filter(Boolean).pop() || $2(tagEl).find(".name").text().trim().toLowerCase().replace(/\s+/g, "-");
          const title2 = $2(tagEl).find(".name").text().trim();
          if (id && title2) tagList.push({ id, title: title2 });
        });
        if (tagList.length > 0) {
          tags.push({
      ...
      const rating = 0;
      const authors = [];
      $2("#tags .tag-container:contains('Artists') .tags a.tag").each((_, el) => {
        const name = $2(el).find(".name").text().trim();
    ...
    async getChapterDetails(chapter) {
      ...
      const $2 = await this.fetchCheerio(request);
      const thumbContainers = $2(".thumbs .thumb-container");
    ...
    async getPopularSectionItems(section, metadata) {
      ...
      const $2 = await this.fetchCheerio(request);
      const items = [];
      $2(".container.index-popular .gallery").each((_, element) => {
        const gallery = $2(element);
        const link = gallery.find("a.cover");
        const href = link.attr("href") || "";
    ...
    async getNewUploadsSectionItems(section, metadata) {
      const page = metadata?.page ?? 1;
      const collectedIds = metadata?.collectedIds ?? [];
      const request = {
      ...
      const items = [];
      $2(".container.index-container .gallery").each((_, element) => {
        if ($2(element).closest(".index-popular").length > 0) return;
        const gallery = $2(element);
        const link = gallery.find("a.cover");
        const href = link.attr("href") || "";
        const mangaId = href.replace(/\/g\/(\d+)\//, "$1");
