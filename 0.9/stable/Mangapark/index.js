...
var source = (() => {
  ...
  // src/Mangapark/main.ts
  var baseUrl = "https://mangapark.io/";
  var MangaparkExtension = class {
    ...
    async getDiscoverSectionItems(section, metadata2) {
      switch (section.id) {
        ...
        case "new_releases_section":
          return this.getNewMangaSectionItems(section, metadata2);
        default:
          return { items: [] };
      }
    }
    ...
    async getSearchResults(query, metadata2, sortingOption) {
      ...
      $2(".flex.border-b.border-b-base-200.pb-5").each((_, element) => {
        ...
        searchResults.push({
          ...
          subtitle,
          metadata: { chapterId }
          // Store chapterId in metadata
        });
      });
      const hasNextPage = !!$2(".btn-accent").next("a").length;
      return {
        items: searchResults,
    ...
    async getChapters(sourceManga) {
      ...
      const $2 = await this.fetchCheerio(request);
      const candidates = [];
      const groupViewsAggregate = /* @__PURE__ */ new Map();
      $2(".px-2.py-2.flex.flex-wrap.justify-between").each((_, element) => {
        const row = $2(element);
        ...
        const publishDate = timestamp ? new Date(parseInt(timestamp)) : void 0;
        const meta = row.find(
        ...
        meta.find(".inline-flex.items-center").filter((_2, el) => $2(el).find("i[name='eye']").length > 0).each((_2, el) => {
          const txt = $2(el).find("span.ml-1").first().text().trim();
          if (txt.includes("+")) {
        ...
        candidates.push({
          ...
          // English by default on this page
          version: groupName,
          groupName,
      ...
      const groupOrder = Array.from(groupViewsAggregate.entries()).sort((a, b) => (b[1] ?? 0) - (a[1] ?? 0)).map(([name]) => name);
    ...
    async getChapterDetails(chapter) {
      const request = {
        url: new URLBuilder(baseUrl).addPath("title").addPath(chapter.sourceManga.mangaId).addPath(chapter.chapterId).build(),
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
      const pages = [];
    ...
    async getPopularSectionItems(section, metadata2) {
      ...
      const $2 = await this.fetchCheerio(request);
      const items = [];
      $2(".relative.w-full.group").each((_, element) => {
        const unit = $2(element);
        ...
        const latestChapterMatch = latestChapter.match(/Chapter (\d+)/);
        const supertitle = latestChapterMatch ? `Ch. ${latestChapterMatch[1]}` : void 0;
        if (title && mangaId && !collectedIds.includes(mangaId)) {
          collectedIds.push(mangaId);
          items.push({
    ...
    async getNewMangaSectionItems(section, metadata2) {
      const searchQuery = {
        title: "",
        filters: []
      };
  ...
  var Mangapark = new MangaparkExtension();
