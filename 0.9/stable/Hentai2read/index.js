...
var source = (() => {
  ...
  // src/Hentai2read/main.ts
  var main_exports = {};
  __export(main_exports, {
    Hentai2read: () => Hentai2read,
    Hentai2readExtension: () => Hentai2readExtension
  });
  init_buffer();
  ...
  // src/Hentai2read/interceptors.ts
  init_buffer();
  var import_types2 = __toESM(require_lib(), 1);
  var Hentai2readInterceptor = class extends import_types2.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers,
  ...
  // src/Hentai2read/main.ts
  var baseUrl = "https://hentai2read.com";
  var Hentai2readExtension = class {
    ...
    async getSearchDetails() {
      try {
        ...
        const category = [];
        const tags = [];
        $2(".col-xs-4.col-sm-3.tag-blocks").each((_, element) => {
          const id = $2(element).find("input[name='chk_wpm_pag_mng_sch_mng_tag_inc[]']").attr("value") ?? "";
          const label = $2(element).find(".block-content.bh-xs.text-ellipsis").text().trim();
          if (label) {
    ...
    async getSearchFilters() {
      ...
      filters2.push({
        id: "tags",
        type: "multiselect",
        options: searchDetails?.tags?.map((t) => ({ id: t.id, value: t.label })) || [],
        value: {},
        allowExclusion: true,
        title: "Tags Filter",
    ...
    async getSearchResults(query, metadata) {
      ...
      $2(".book-grid-item-container").each((_, element) => {
        if ($2(element).find(".book-grid-item a[href*='hive.arf.dev']").length > 0) {
          return;
        }
        const titleLink = $2(element).find(".title, .overlay-title a").first();
        const href = titleLink.attr("href") || "";
        const mangaId = href.replace(/^https?:\/\/hentai2read\.com\//, "").replace(/\/$/, "");
        const title = $2(element).find(".title-text, .overlay-title a").first().text().trim();
        const subtitle = $2(element).find(".overlay-sub div").first().text().trim() || void 0;
        let image = "";
    ...
    async getMangaDetails(mangaId) {
      ...
      const $2 = await this.fetchCheerio(request);
      const title = $2(".tagButton:contains('Read')").text().trim() || $2(".title .pretty").first().text().trim() || $2("title").text().split(" | ")[0].trim();
      const altTitles = [];
      $2(".title").each((_, el) => {
        const alt = $2(el).text().trim();
        if (alt && alt !== title) altTitles.push(alt);
      });
      let image = "";
      const imgElement = $2("#js-linkNext img.img-responsive, .img-container img.img-responsive").first();
      image = imgElement.attr("data-src") || imgElement.attr("src") || "";
      if (image && image.startsWith("//")) {
        image = "https:" + image;
      }
      const description = $2(".text-muted:contains('Nothing yet')").length > 0 ? "" : $2("li.text-primary:contains('Storyline') p").text().trim();
      let status = "UNKNOWN";
      const statusText = $2("li.text-primary:contains('Status') .tagButton").text().trim().toLowerCase();
      ...
      const tags = [];
      const categoryTags = [];
      $2("li.text-primary:contains('Category') .tagButton").each((_, el) => {
        const href = $2(el).attr("href") || "";
        const id = href.split("/").filter(Boolean).pop() || "";
        const title2 = $2(el).text().trim();
        if (id && title2) categoryTags.push({ id, title: title2 });
      });
      ...
      const contentTags = [];
      $2("li.text-primary:contains('Content') .tagButton").each((_, el) => {
        const href = $2(el).attr("href") || "";
        const id = href.split("/").filter(Boolean).pop() || "";
        const title2 = $2(el).text().trim();
        if (id && title2) contentTags.push({ id, title: title2 });
      });
      ...
      const authorTags = [];
      $2("li.text-primary:contains('Author') .tagButton").each((_, el) => {
        const href = $2(el).attr("href") || "";
        const id = href.split("/").filter(Boolean).pop() || "";
        const title2 = $2(el).text().trim();
        if (id && title2) authorTags.push({ id, title: title2 });
      });
      ...
      const artistTags = [];
      $2("li.text-primary:contains('Artist') .tagButton").each((_, el) => {
        const href = $2(el).attr("href") || "";
        const id = href.split("/").filter(Boolean).pop() || "";
        const title2 = $2(el).text().trim();
        if (id && title2) artistTags.push({ id, title: title2 });
      });
      ...
      let rating = 0;
      const ratingText = $2(".js-raty").attr("data-score") || "0";
    ...
    async getChapters(sourceManga) {
      ...
      const $2 = await this.fetchCheerio(request);
      const chapters = [];
      $2(".nav-chapters li").each((_, element) => {
        const chapterLink = $2(element).find("a.pull-left");
        ...
        const dateText = $2(element).find(".text-muted small").text().trim();
    ...
    async getChapterDetails(chapter) {
      ...
      const $2 = await this.fetchCheerio(request);
      const pages = [];
      const scriptContent = $2("script").filter((_, el) => {
        return ($2(el).html() || "").includes("var gData");
      }).html() || "";
      if (scriptContent) {
    ...
    async getStaffPickSectionItems(section, metadata) {
      const page = metadata?.page ?? 1;
      ...
      const $2 = this.baseHtml;
      const items = [];
      $2(".row.book-grid > div:not(:last-child)").each((_, element) => {
        const gallery = $2(element).find(".book-grid-item");
    ...
    async getReadingNowSectionItems(section, metadata) {
      const page = metadata?.page ?? 1;
      const collectedIds = metadata?.collectedIds ?? [];
      if (!this.baseHtml) {
        console.warn("Base HTML not initialized. Returning empty section.");
      ...
      const $2 = this.baseHtml;
      const items = [];
      $2(".block-header:contains('Reading Now') + .block-content .nav-users li").each((_, element) => {
        const link = $2(element).find("a.link-effect");
        const href = link.attr("href") || "";
        const mangaId = href.replace("https://hentai2read.com/", "").replace(/\/$/, "");
        ...
        const title = link.contents().filter((_2, node) => node.type === "text" && $2(node).text().trim() !== "").text().trim();
        if (title && mangaId && !collectedIds.includes(mangaId)) {
          collectedIds.push(mangaId);
          items.push(
            createDiscoverSectionItem({
    ...
    async getReaderRecommendationSectionItems(section, metadata) {
      const page = metadata?.page ?? 1;
      const collectedIds = metadata?.collectedIds ?? [];
      if (!this.baseHtml) {
        console.warn("Base HTML not initialized. Returning empty section.");
      ...
      const $2 = this.baseHtml;
      const items = [];
      $2(".block-header:contains('Reader Recommendation') + .block-content .nav-users li").each((_, element) => {
        const link = $2(element).find("a.link-effect");
        const href = link.attr("href") || "";
        const mangaId = href.replace("https://hentai2read.com/", "").replace(/\/$/, "");
        const imgElement = link.find("img.img-avatar");
        ...
        const title = link.contents().filter((_2, node) => node.type === "text" && $2(node).text().trim() !== "").text().trim();
        const heartIcons = $2(element).find(".js-rating i.fa-heart.text-city").length;
    ...
    async getRecentlyUploadedSectionItems(section, metadata) {
      const page = metadata?.page ?? 1;
      const collectedIds = metadata?.collectedIds ?? [];
      const request = {
        url: baseUrl,
      ...
      const $2 = await this.fetchCheerio(request);
      const items = [];
      $2(".block-content .nav-users li").each((_, element) => {
        const link = $2(element).find("a.link-effect").first();
        const href = link.attr("href") || "";
        const mangaId = href.replace("https://hentai2read.com/", "").replace(/\/$/, "");
        const imgElement = link.find("img.img-avatar");
        let image = imgElement.attr("src") || "";
        ...
        const title = link.contents().filter((_2, node) => {
          return node.type === "text" && $2(node).text().trim() !== "";
        }).first().text().trim();
        const subtitleText = $2(element).find(".text-ellipsis").first().text().trim();
        const subtitle = subtitleText.replace(/\s+/g, " ").replace(/^\d+\s*→\s*/, "").replace(/\s+\d+\s*days?\s*ago$/, "");
        const chapterId = $2(element).find(".text-ellipsis a").first().attr("href")?.replace("https://hentai2read.com/", "").replace(/\/$/, "") || "";
        if (title && mangaId && !collectedIds.includes(mangaId)) {
          collectedIds.push(mangaId);
          items.push({
  ...
  var Hentai2read = new Hentai2readExtension();
