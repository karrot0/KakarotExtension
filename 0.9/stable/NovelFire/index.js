...
var source = (() => {
  ...
  // src/NovelFire/main.ts
  var main_exports = {};
  __export(main_exports, {
    NovelFire: () => NovelFire,
    NovelFireExtension: () => NovelFireExtension
  });
  init_buffer();
  ...
  // src/NovelFire/interceptors.ts
  init_buffer();
  var import_types2 = __toESM(require_lib(), 1);
  var NovelFireInterceptor = class extends import_types2.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers,
  ...
  // src/NovelFire/main.ts
  var baseUrl = "https://novelfire.net";
  var apiUrl = "https://novels-83gm.onrender.com";
  var NovelFireExtension = class {
    ...
    async getSearchResults(query, _metadata) {
      if (!query.title || query.title.trim() === "") {
        ...
        const results = await this.getPopularSectionItems(section, void 0);
        const items2 = results.items.filter((item) => "mangaId" in item && "title" in item && "imageUrl" in item).map((item) => ({
          mangaId: item.mangaId,
          title: item.title,
          imageUrl: item.imageUrl,
          subtitle: item.supertitle || item.subtitle,
      ...
      const jsonString = Application.arrayBufferToUTF8String(data2);
      const result = JSON.parse(jsonString);
      const $2 = load(result.html);
      const items = [];
      $2(".novel-item").each((_, el) => {
        const novel = $2(el);
    ...
    async getMangaDetails(mangaId) {
      ...
      const title = String($2(".novel-title").text()).trim();
      const coverUrl = String($2(".novel-header img").attr("src")) || "";
      const genres = [];
      $2(".categories .property-item").each((_, el) => {
        genres.push(String($2(el).text()).trim());
      });
      const tags = [];
      $2(".tag").each((_, el) => {
        tags.push(String($2(el).text()).trim());
      });
      const rating = parseFloat(String($2(".rating").text()).trim()) || 1;
      const author = String($2(".property-item").first().text()).trim();
      const status = String($2(".completed").text()).trim() || "UNKNOWN";
      const description = String($2(".content.expand-wrapper").text()).trim();
      const tagGroups = [];
      if (genres.length > 0) {
        tagGroups.push({
          id: "genres",
    ...
    async getChapters(sourceManga) {
      ...
      const request = {
        url: new URLBuilder(baseUrl).addPath("book").addPath(mangaId).addPath("chapters").build(),
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
      const chapters = [];
      $2(".chapter-list li").each((_, el) => {
        const li = $2(el);
        const a = li.find("a");
        const url = String(a.attr("href")) || "";
        const title = String(a.attr("title")) || a.text().trim();
        const chapterNo = parseFloat(String(a.find(".chapter-no").text()).trim()) || void 0;
        ...
        chapters.push({
          ...
          sourceManga,
          chapNum: chapterNo ?? 0,
          publishDate: createdAt ? new Date(createdAt) : void 0,
          volume: 0,
          langCode: "en",
          version: "1"
        });
    ...
    async getChapterDetails(chapter) {
      const request = {
        url: new URLBuilder(baseUrl).addPath("book").addPath(chapter.chapterId).build(),
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
      const contentDiv = $2("#content");
      contentDiv.find(".nf-ads, .adcash, iframe").remove();
      contentDiv.find("*").each((_, el) => {
        if ($2(el).prop("tagName")?.startsWith("AZ")) {
          $2(el).remove();
        }
      });
      const textContent2 = contentDiv.html() || "";
      const apiRequest = {
        url: apiUrl + "/create_images",
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ content: textContent2 })
      };
      const [, data2] = await Application.scheduleRequest(apiRequest);
      const jsonString = Application.arrayBufferToUTF8String(data2);
      const dataObj = JSON.parse(jsonString);
      if (dataObj.status !== "success") {
        throw new Error(dataObj.error || "Failed to generate images");
      }
      const imageUrls = dataObj.data.images.map((url) => apiUrl + url);
      return {
    ...
    async getPopularSectionItems(section, metadata) {
      ...
      const request = {
        url: new URLBuilder(baseUrl).addPath("genre-all").addPath("sort-popular").addPath("status-all").addPath("all-novel").addQuery("page", page.toString()).build(),
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
      const items = [];
      $2(".novel-item").each((_, el) => {
        ...
        const coverUrl = novel.find("img").attr("data-src") || novel.find("img").attr("src") || "";
        const rating = parseFloat(novel.find(".badge._br").text().trim()) || void 0;
        const chapters = parseInt(novel.find(".novel-stats").text().replace(/\D/g, "")) || void 0;
        ...
        if (title && mangaId && !collectedIds.includes(mangaId)) {
          ...
          items.push({
            ...
            imageUrl: coverUrl,
            title,
            supertitle: rating ? `\u2B50 ${rating}` : void 0,
            metadata: void 0
          });
        }
      });
      const hasNextPage = !!$2(".pagination .page-item.active + .page-item").length;
      return {
        items,
    ...
    async getUpdatedSectionItems(section, metadata) {
      ...
      const collectedIds = metadata?.collectedIds ?? [];
      const request = {
        url: new URLBuilder(baseUrl).addPath("genre-all").addPath("sort-latest-release").addPath("status-all").addPath("all-novel").addQuery("page", page.toString()).build(),
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
      const items = [];
      $2(".novel-item").each((_, el) => {
        ...
        if (title && mangaId && !collectedIds.includes(mangaId)) {
          ...
          items.push({
            ...
            imageUrl: coverUrl,
            title,
            subtitle: chapters ? `${chapters} chapters` : void 0,
            metadata: void 0
          });
        }
      });
    ...
    async getNewSectionItems(section, metadata) {
      const page = metadata?.page ?? 1;
      const collectedIds = metadata?.collectedIds ?? [];
      const request = {
        url: new URLBuilder(baseUrl).addPath("genre-all").addPath("sort-new").addPath("status-all").addPath("all-novel").addQuery("page", page.toString()).build(),
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
      const items = [];
